...
class DynaMOSATestStrategy(AbstractMOSATestStrategy):
    ...
    def generate_tests(self) -> tsc.TestSuiteChromosome:
        ...
        while self.resources_left() and len(self._archive.uncovered_goals) > 0:
            if(i==50 or i==100):
                self.evolve_rand(self.create_test_suite(self._archive.solutions))
            # else:
            #     if(i==50):
    ...
    def evolve_rand(self, test_suite: tsc.TestSuiteChromosome):
        ...
        #
        test_cases = languagemodelseeding.target_rand_functions(  # 函数，为test_suit覆盖较少的函数生成测试用例
            test_suite,
