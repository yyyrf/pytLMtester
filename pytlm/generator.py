...
from pynguin.callllm import call_model
...
def _setup_language_model_seeding(
    ...
    if (
        ...
        if (
            config.configuration.pytlm.authorization_key == ""
            or config.configuration.pytlm.model_name == ""
        ) and (config.configuration.pytlm.replay_generation_from_file == ""):
            _LOGGER.error(
        ...
        if config.configuration.pytlm.replay_generation_from_file:
            if not os.path.isfile(
                config.configuration.pytlm.replay_generation_from_file
            ):
                _LOGGER.error(
                ...
                return False
            call_model.languagemodel = call_model.FileMockedModel(
                config.configuration.pytlm.replay_generation_from_file
            )
        else:
            call_model.languagemodel.temperature = config.configuration.pytlm.temperature
            call_model.languagemodel.authorization_key = (
                config.configuration.pytlm.authorization_key
            )
            call_model.languagemodel.complete_model = (
                config.configuration.pytlm.model_name
            )
            call_model.languagemodel.model_base_url = (
                config.configuration.pytlm.model_base_url
            )
            call_model.languagemodel.model_relative_url = (
                config.configuration.pytlm.model_relative_url
            )
    ...
    with open(module_path, encoding="UTF-8") as module_file:
        call_model.languagemodel.test_src = module_file.read()
    seeding.languagemodelseeding.model = call_model.languagemodel
...
def _setup_and_check() -> tuple[TestCaseExecutor, TestCluster] | None:
    ...
    if not _setup_language_model_seeding(test_cluster, executor):
        return None
